...
class RRT():
    ...
    def search(self):
        ...
        if temp > 0:
            # random点を打つ(0, 1の範囲で選ぶ)
            s_x = np.random.rand() *self.MAX_x / 2
            s_y = np.random.rand() *self.MAX_y / 2
...
def main(goal_pos):
    ...
    goal = goal_pos
    init = [0.0, 0.0]
    obstacles = np.array([[0, 0.5, 0.15], [0.1, 0.3, 0.15], [0.5, 0.0, 0.15], [0.0, 0.2, 0.15]])
    ...
    #print("this is final path {}".format(final_path))
